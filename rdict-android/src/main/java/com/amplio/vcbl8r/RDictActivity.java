...
import com.amplio.vcbl8r.review.ReviewManager;
import com.amplio.vcbl8r.review.StatisticsManager;
import com.amplio.vcbl8r.search.AssetInputStreamProvider;
import com.amplio.vcbl8r.search.Dictionary;
import com.amplio.vcbl8r.setup.DictionaryDownloader;
import com.amplio.vcbl8r.setup.DownloadService;
import com.amplio.vcbl8r.setup.SetupActivity;
...
public class RDictActivity extends TabActivity implements  AssetInputStreamProvider, OnClickListener {
	...
	private void initDatabaseManagers() {
		if(odb == null || odb.isClosed()) {
		    odb = ODBFactory.open("/sdcard/vcbl8r/" + "vcbl8r_db.odb" );
	    ...
	    if(con == null || ! con.isOpen()) {
	    	con = SQLiteDatabase.openDatabase("/sdcard/vcbl8r/word.db", null, SQLiteDatabase.OPEN_READWRITE | SQLiteDatabase.CREATE_IF_NECESSARY);
